...
if [[ "$stage" != build ]]; then
    ...
    # kmod loaded here) and the whole .bpfman script corpus, so an
    # lsm script that misses the TestLsm_ naming convention still
    # runs on a bpf-LSM kernel (the default !external selector still
    # applies).
